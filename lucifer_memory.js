...
export default {
  ...
  async onEnable(ctx) {
    try {
      const basePath = path.resolve('./llm_memory');
      const userMemoryPath = path.join(basePath, 'user_memory.json');
      const activeMemoryPath = path.join(basePath, 'active_memory.json');

      const userMemory = JSON.parse(readFileSync(userMemoryPath, 'utf-8'));
      const activeMemory = JSON.parse(readFileSync(activeMemoryPath, 'utf-8'));
